...
using Service.Analytics.WebApi.Extensions;
using Service.Analytics.WebApi.Middlewares;
using Service.Analytics.WebApi.Options;
using Service.Analytics.WebApi.Utility;
...
LoggingUtility.Run(() =>
{
	...
	builder.Services
		...
		.InstallServicesFromAssemblies(
			...
			Service.Analytics.WebApi.AssemblyReference.Assembly,
			Authorization.AssemblyReference.Assembly,
			Persistence.AssemblyReference.Assembly)
		.InstallModulesFromAssemblies(
				builder.Configuration,
				Service.Analytics.Infrastructure.AssemblyReference.Assembly);
	...
	{
		var applicationDbContext = webApplication.Services.CreateScope().ServiceProvider.GetRequiredService<AnalyticsDbContext>();
