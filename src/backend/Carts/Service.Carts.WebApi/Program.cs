...
using Service.Carts.WebApi.Extensions;
using Service.Carts.WebApi.Middlewares;
using Service.Carts.WebApi.Options;
using Service.Carts.WebApi.Utility;
...
LoggingUtility.Run(() =>
{
	...
	builder.Services
		...
		.InstallServicesFromAssemblies(
			...
			Service.Carts.WebApi.AssemblyReference.Assembly,
			Authorization.AssemblyReference.Assembly,
			Persistence.AssemblyReference.Assembly)
		.InstallModulesFromAssemblies(
				builder.Configuration,
				Service.Carts.Infrastructure.AssemblyReference.Assembly);
	...
	{
		var applicationDbContext = webApplication.Services.CreateScope().ServiceProvider.GetRequiredService<CartDbContext>();
