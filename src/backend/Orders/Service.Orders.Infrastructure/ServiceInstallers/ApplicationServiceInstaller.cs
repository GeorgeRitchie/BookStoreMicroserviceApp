...
namespace Service.Orders.Infrastructure.ServiceInstallers
{
	...
	internal sealed class ApplicationServiceInstaller : IServiceInstaller
	{
		...
		public void Install(IServiceCollection services, IConfiguration configuration) =>
			services
				.AddMediatR(cfg => cfg.RegisterServicesFromAssemblies(OrderApplication.AssemblyReference.Assembly))
				...
				.AddValidatorsFromAssembly(
					OrderApplication.AssemblyReference.Assembly,
				...
				.AddAutoMapper(config => config.AddProfile(new AssemblyMappingProfile(OrderApplication.AssemblyReference.Assembly)))
